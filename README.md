...
Rubys `Object#tap` is a awesome. mighty_tap tries to make it even better by adding some missing features, while maintining full compatibility with the orginal. In order to make its usage more pleasant, `mighty_tap` is defined as in instance method on `Object` and aliased to `mtap`.
